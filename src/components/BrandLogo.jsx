import React from 'react';

/**
 * Official LivGruha Brand Logo Component
 * High-definition rendering of the exact official brand identity from the client PDF.
 */
export default function BrandLogo({ 
  variant = 'default', 
  size = 'default', 
  className = '' 
}) {
  const isDark = variant === 'dark'; // For dark backgrounds (e.g. Footer)

  // Height configurations for prominent, crystal-clear display
  const heightClasses = {
    small: 'h-10 sm:h-12',
    default: 'h-14 sm:h-16 md:h-[72px]',
    large: 'h-20 sm:h-24',
    footer: 'h-20 sm:h-24'
  };

  const h = heightClasses[size] || heightClasses.default;
  const logoSrc = isDark ? '/livgruha-logo-white.png' : '/livgruha-logo-transparent.png';

  return (
    <div className={`inline-flex items-center select-none cursor-pointer group ${className}`}>
      <img
        src={logoSrc}
        alt="LIVGRUHA INTERIORS - Designing Spaces. Enriching Lives."
        className={`${h} w-auto object-contain transition-transform duration-300 group-hover:scale-105`}
        style={{
          imageRendering: '-webkit-optimize-contrast',
          filter: isDark 
            ? 'drop-shadow(0 2px 10px rgba(0,0,0,0.5))' 
            : 'drop-shadow(0 2px 6px rgba(155,63,35,0.08))'
        }}
        loading="eager"
      />
    </div>
  );
}
